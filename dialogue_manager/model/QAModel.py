...
class QAModel(model):
    def __init__(self, num_actions):
        super(QAModel, self).__init__(num_actions)
        self.simu_flag = True
        self.successes = [0, 0]
        self.rewards = []
    ...
    def get_next_action(self, cur_state):
        ...
        flag = True
        if action == 1 and state.intent == 6:
            sorted_ind = np.argsort(state.distribution)[::-1]
            if 0 in sorted_ind[:2]:
                flag = False
            
        if flag and (action == 1 or action == 5) and state.intent in [2,6] and \
            random.random() <= float(os.getenv('PARSE_MIS')):
            action = 0
